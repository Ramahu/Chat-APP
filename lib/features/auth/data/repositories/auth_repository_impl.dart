...
class AuthenticationRepositoryImp implements AuthenticationRepository{
  ...
  Future<Either<Failure, UserCredential>> signUp(SignUpEntity signUp) async {
    if ( !await networkInfo.isConnected) {
      ...
      try{
        final signUpModel = SignUpModel(name: signUp.name, email: signUp.email,
            password: signUp.password, repeatedPassword: signUp.repeatedPassword);
  ...
  @override
  Future<Either<Failure, Unit>> createUser(SignUpEntity user) async {
    if (await networkInfo.isConnected) {
      try {
        final signUpModel = SignUpModel(uid:user.uid,name: user.name, email: user.email,
            password: user.password, repeatedPassword: user.repeatedPassword);
        await authRemoteDataSource.createUser(signUpModel);
